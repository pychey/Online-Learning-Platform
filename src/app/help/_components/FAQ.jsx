  ...
  return (
    <div className="mt-6 bg-gray-100 text-xl rounded-sm">
      {items.map((item, index) => (
        <div key={index}>
          <div 
            className={`flex justify-start items-center gap-4 px-4 py-3 opacity-70 cursor-pointer ${index < items.length-1 ? 'border-b border-gray-300' : ''}`} 
            onClick={() => toggle(index)}
          >
            <span className={`${activeIndex === index ? 'text-3xl' : `text-2xl`}`}>{activeIndex === index ? '-' : '+'}</span>
            {item.title}
          </div>

                        ...
                        <div className="bg-gray-100 w-[75%] mx-auto px-2 rounded-lg">
                            ...
                            return(
                                ...
                                <div key={index} className="flex items-center justify-start  gap-4 py-4  my-1">
                                    <div className={`relative border-2 border-gray-400  rounded-2xl h-4 aspect-square`}><Tick className={` h-[17px] w-[17px] ${l.isCompleted?"block":"hidden"} absolute -left-[2.3px] -top-0.5 text-white bg-green-300 rounded-2xl`}/></div>
                                    <Link href={`/lesson/${l.slug}`} className={`${isActiveLesson?`font-semibold`:`font-light`} `}>{index+1}. {l.title}</Link>
                                </div>
                            )})}
                        </div>
                    )}
